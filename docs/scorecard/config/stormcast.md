...
Each event is a severe convective episode of 2025, a time window over the
model domain. Initial conditions run every 3 hours from 12 hours before
the window to its end. The headline curves pool every event's initial
conditions, and the Event selector shows one episode at a time. Truth is
the HRRR analysis, and scores are uniformly weighted on the model grid.

## Reference

StormCast, a kilometer-scale convection-allowing model emulated with
generative diffusion, 2024. Paper: <https://arxiv.org/abs/2408.10958>.
