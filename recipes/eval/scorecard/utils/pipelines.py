...
class RegionalForecastPipeline(ForecastPipeline):
    ...
    Two things separate such models (StormCast is the configured example)
    from the ones the stock pipeline drives.  Their initial conditions and
    truth come from a source that returns its whole grid while the model
    runs on a window of it, so the pipeline crops every store, and a live
    ``verification_source``, with :class:`SubgridSource` (a live
    ``ic_source`` needs no help: the recipe selects the window itself).
